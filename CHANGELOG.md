...
**Archivos:** `def/country/[estado]/speed_limits.rrmx.sii` y `traffic.rrmx.sii` (NUEVO)
Se crearon definiciones específicas por estado para 12 estados de Reforma (`aguascalientes`, `baja_california`, `baja_california_sur`, `chihuahua`, `coahuila`, `durango`, `jalisco`, `nayarit`, `san_luis_potosi`, `sinaloa`, `sonora`, `zacatecas`). Se utiliza la terminación `.rrmx.sii` para asegurar máxima compatibilidad con vehículos propios de Reforma (como patrullas o autobuses locales).

**1. Límites de velocidad realistas (`speed_limits.rrmx.sii`)**
...
**2. Composición de tráfico adaptada (`traffic.rrmx.sii`)**
...
### 🇺🇸 Mejora de Contraste para EE.UU.
**Archivos:** `def/country/[estado]/traffic.rrmx.sii` (NUEVO)
Para maximizar el contraste al cruzar la frontera, se aplicaron configuraciones complementarias a los 20 estados base de EE.UU.:
- **Aumento masivo de lujo/deportivos:** Tesla (3.00x), Mustang (2.50x), Mercedes CE (2.00x).
- **Reducción de autos viejos:** Mercury (0.30x), Pickups Chevy antiguas (0.50x).
- El contraste al salir de México hacia EE.UU. será inmediatamente notable por la calidad y modernidad de los vehículos en la carretera.
